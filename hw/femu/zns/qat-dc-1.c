...
CpaStatus qat_dc_compress(FemuCtrl *n, uint32_t inst_idx, void *input, uint32_t input_len, uint32_t *output_len, uint32_t count)
{
    ...
    while (left)
    {
        ...
        if (CPA_STATUS_SUCCESS == status)
        {
            ...
            for (uint32_t i = 0; i < batch_sz; i++)
            { 
                if(pOpDatas[inst_idx * QAT_OP_PER_INST + i]->results.produced > input_len){
...
CpaStatus qat_dc_decompress(FemuCtrl *n, uint32_t inst_idx, void *input, uint32_t input_len, void* output, uint32_t *output_len, uint32_t count, uint64_t onepagesize, uint64_t gap)
{
    ...
    while (left)
    {
        ...
        if (CPA_STATUS_SUCCESS != status)
        {
            qat_error("cpaDcDpEnqueueOpBatch Decomp failed. (status = %d)\n", status);
        }
...
//源代码中解压操作有Compare with original data，这里没有实现
//考虑到实现的话需要在原函数中增加一个参数SampleData *sample_data,感觉比较冗杂
//如果后续想测试压缩和解压的准确性，用脚本测试即可